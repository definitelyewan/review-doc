...
export const load = async (loadEvent) => {
    ...
    newest_reviews = await Promise.all(newest_reviews.map(async review => {
        let user = await db.query(`SELECT user_id, user_name FROM user WHERE user_id = ${review.user_id} LIMIT 1`);
    ...
    default: async ({ request }) => {
        if (request.method === 'POST') {
            ...
            if (type === 'update') {
                ...
                const igdb_json = await igdb.query(`fields *; search "${media_search_word}"; where (version_parent = null & (category = 0 | category = 10 | category = 8));`);
